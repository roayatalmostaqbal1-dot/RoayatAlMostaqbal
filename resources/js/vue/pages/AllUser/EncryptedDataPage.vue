      ...
      <div class="lg:col-span-2">
        <Card>
          ...
          <div v-if="activeTab === 'encrypt'" class="space-y-6">
            ...
            <!-- Success Message -->
            <div v-if="encryptSuccess" class="p-4 rounded-lg bg-green-500 bg-opacity-10 border border-green-500 text-green-400">
          ...
          <div v-if="activeTab === 'decrypt'" class="space-y-6">
            <div v-if="!isDecrypted">
              <p class="text-gray-400 mb-6">Enter your password to decrypt your stored data.</p>
            ...
            <div v-else>
              ...
              <div class="p-4 rounded-lg bg-[#1f3a4a] border border-[#3b5265]">
                ...
                <div class="bg-[#162936] p-4 rounded-lg border border-[#3b5265] text-white whitespace-pre-wrap break-words max-h-96 overflow-y-auto">
                  {{ decryptedData }}
                </div>
              </div>

              <div class="mt-4 p-4 rounded-lg bg-blue-500 bg-opacity-10 border border-blue-500 text-blue-400 text-sm">
                ℹ️ Your decrypted data is displayed here. It will be cleared from memory when you navigate away.
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'view'" class="space-y-6">
            <p class="text-gray-400 mb-6">View information about your encrypted data stored on the server.</p>

            <Button
              variant="primary"
              @click="handleFetchStoredData"
              :disabled="isLoading"
            >
            ...
            <div v-if="storedDataInfo" class="space-y-4">
              ...
              <div class="p-4 rounded-lg bg-yellow-500 bg-opacity-10 border border-yellow-500 text-yellow-400 text-sm">
...
const handleEncrypt = async () => {
  ...
  try {
    // Encrypt the data
    const encryptedData = await encryptUserData(encryptPassword.value, {
      text: textToEncrypt.value,
      timestamp: new Date().toISOString(),
    });

    // Store on server
    const response = await apiClient.post('/encrypted-data', {
      ...encryptedData,
      data_type: 'user_text',
    });

    if (response.data.success) {
      toastStore.success('Success', 'Data encrypted and stored successfully!');
...
const handleDecrypt = async () => {
  ...
  try {
    ...
    decryptedData.value = decrypted.text;
    isDecrypted.value = true;
    toastStore.success('Success', 'Data decrypted successfully!');
  } catch (err) {
